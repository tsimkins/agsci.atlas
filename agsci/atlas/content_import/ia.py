...
# Based on JSON input, create filter sets.
def createFilterSets(context):
    # Traverse to the JSON static configuration
    resource = context.restrictedTraverse('++resource++agsci.atlas/filter_sets.json')

    # Get the config contents
    data = json.loads(resource.GET())
    
    # Create "Filters" container folder if one doesn't exist
    base_id = 'filters'

    # If the short name exists, grab the object
    if base_id in context.objectIds():
        container = context[base_id]

    # Otherwise, create a folder under `context`
    else:
        container = createContentInContainer(context, "Folder" , id=base_id, title="Filters")
    ...
    for k in sorted(data.keys()):
        ...
        # Get the filter name, and ploneify it
        name = k.strip()
        _id = ploneify(name)
        
        # Add a Filter Set Item
        if _id in container.objectIds():
            item = container[_id]
        else:
            item = createContentInContainer(container, 'atlas_filter_set', id=_id, title=name)
        
        item.atlas_filters = tuple(sorted(v))