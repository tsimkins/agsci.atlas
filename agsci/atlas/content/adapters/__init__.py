...
class PDFDownload(BaseAtlasAdapter):
    ...
    def pdf_autogenerate(self):
        return not not getattr(self.context, 'pdf_autogenerate', None)

    # Check for a PDF download or an auto-generated PDF
    def hasPDF(self):
        return not not (self.pdf_file or self.pdf_autogenerate)

    def isPreview(self):
        return not not self.context.REQUEST.form.get('preview', False)

    # Return the PDF data and filename, or (None, None)
    def getPDF(self):

        if self.hasPDF() or self.isPreview():
            ...
            elif self.pdf_autogenerate or self.isPreview():
