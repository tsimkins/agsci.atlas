...
# Check for large product sizes
class LargeProductSize(ContentCheck):

    title = "Large Products"

    description = "Checks for products containing large files."

    action = "Review files inside product to see if they can be reduced in size."
    ...
    def get_sizes(self, context):

        sizes = []

        for field in self.fieldnames:

            _ = self.get_size(context, field)
    ...
    def get_size(self, context, field):

        if hasattr(context.aq_base, field):
            _ = getattr(context.aq_base, field)

            if isinstance(_, (NamedBlobImage, NamedBlobFile)):

                if hasattr(_, 'size') and _.size:
                    return _.size
    ...
    def check(self):
        ...
        # Sum of sizes of all binary fields, in KB
        total_size = sum([x.size for x in data])/1024.0
...
class AutogeneratedArticleInternalLinks(AutogeneratedArticleCheck):
    ...
    def value(self):
        return [x for x in self.soup.findAll('a') if self.resolveuid_re.search(x.get('href', ''))]
